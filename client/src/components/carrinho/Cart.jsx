  ...
  const handleClearCart = async () => {
    try {
        if (cartItems.length === 0) {
            ...
            // Ask user for confirmation before clearing the cart
            const confirmClear = window.confirm('Tem certeza que deseja limpar o carrinho?');

            if (!confirmClear) {
                return; // If user cancels, do nothing
            }

            // Remove dataQtdeSizeVal from localStorage if it exists
